...
float walk = 0.0;     // Initial rotation angle for walking animation
float rate = 1000;   // Rate of change for the walking animation
float dwalk = 0.005;  // Change in walk angle

float wave = 0.0;     // Initial rotation angle for waving animation
float dwave = 0.01;   // Change in wave angle
...
void display() {
    ...
    int modelLoc;                   // shader location of the model matrix
    ...
    /* draw the right leg with walking animation */
    ...
    /* draw the left leg with walking animation */
    matrixStack.push(model);
    model = glm::rotate(model, -walk, glm::vec3(1.0, 0.0, 0.0));
    model = glm::rotate(model, 2.3f, glm::vec3(0.0, 1.0, 0.0));
    glUniformMatrix4fv(modelLoc, 1, 0, glm::value_ptr(model));
    ...
    /* draw the right arm with waving animation */
    matrixStack.push(model);
    model = glm::translate(model, glm::vec3(-0.1, 0.0, 1.0));
    model = glm::rotate(model, -1.0f + wave, glm::vec3(0.0, 1.0, 0.0));
    glUniformMatrix4fv(modelLoc, 1, 0, glm::value_ptr(model));
    ...
    /* draw the left arm with waving animation */
    matrixStack.push(model);
    model = glm::translate(model, glm::vec3(0.1, 0.0, 1.0));
    model = glm::rotate(model, 1.0f + wave, glm::vec3(0.0, 1.0, 0.0));
    glUniformMatrix4fv(modelLoc, 1, 0, glm::value_ptr(model));
    ...
    // Update waving animation parameters
    wave += dwave;
    if (wave > 0.5)
        dwave = -0.01;
    if (wave < -0.5)
...
int main(int argc, char** argv) {
    ...
    // start by setting an error callback in case something goes wrong
    ...
    // GLFW main loop, display model, swap buffer and check for input
